...
def generate_dataset_D1(num_samples=8000):
    ...
    for i in range(len(centers_x)):
        distances = np.sqrt(
            (points[:, 0] - centers_x[i]) ** 2 + (points[:, 1] - centers_y[i]) ** 2
        )
...
def generate_dataset_D2(num_samples=8000):
    ...
    def generate_torus_points(
        R, r, n_theta, n_phi, center=np.array([0, 0, 0]), rotation=None
    ):
    ...
    for center in grid_positions:
        ...
        rotation_b = np.array([[1, 0, 0], [0, 0, 1], [0, 1, 0]])  # Interlock
...
def visualize_dataset(X, y, dataset_name, dim=3):
    ...
    if dim == 2:
        ...
        ax.scatter(
            X[y == 0, 0], X[y == 0, 1], X[y == 0, 2], alpha=0.5, label="Class a (Ma)"
        )
        ax.scatter(
            X[y == 1, 0], X[y == 1, 1], X[y == 1, 2], alpha=0.5, label="Class b (Mb)"
        )
...
def perform_topology_analysis(
    network, dataloader, class_idx=0, k=15, threshold=2.5, max_dim=2
):
    """
    ...
    for layer_idx, activations in enumerate(layer_activations):
        if activations.size == 0:
            # If empty, skip
            print(
                f"Layer {layer_idx+1} had no activations for class {class_idx}. Skipping."
            )
...
def visualize_topology_change(
    layer_betti, activation_name, dataset_name, class_name="Ma"
):
    """
    ...
    plt.title(
        f"Topology Across Layers ({activation_name}, {dataset_name}, {class_name})"
    )
    plt.xlabel("Layer")
    ...
    plt.tight_layout()
    plt.savefig(
        f"topology_change_{activation_name}_{dataset_name}_{class_name}.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def generate_and_train(config):
    """Main entry training function"""
    ...
    for dataset_name, (X, y) in datasets.items():
        ...
        for activation in activations:
            ...
            plt.grid(True, linestyle="--", alpha=0.7)
            plt.savefig(
                f"loss_curves_{dataset_name}_{activation}.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
            ...
            visualize_topology_change(
                ma_betti, activation, dataset_name, class_name="Ma"
            )
            ...
            visualize_topology_change(
                mb_betti, activation, dataset_name, class_name="Mb"
            )
