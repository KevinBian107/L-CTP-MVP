...
def make_env(env_id, seed, idx, capture_video, run_name):
    def thunk():
        ...
        env = gym.wrappers.TransformReward(
            env, lambda reward: np.clip(reward, -100, 100)
        )
...
def compute_graph_geodesic_distance(X, k=15):
    ...
    nbrs = NearestNeighbors(n_neighbors=min(k, X.shape[0]), algorithm="ball_tree").fit(
        X
    )
...
def analyze_topology(agent, env, num_samples=500, k=15, threshold=2.5):
    ...
    results = {
        "input": {
            "ph": compute_persistent_homology(
                states, k=k, max_dim=min(2, states.shape[1] - 1)
            ),
            "data": states,
        },
        "actor": [],
    ...
    for i, act in enumerate(agent.actor_activations):
        ph_results = compute_persistent_homology(
            act, k=k, max_dim=min(2, act.shape[1] - 1)
        )
        betti = compute_betti_numbers(ph_results, threshold=threshold)
        results["actor"].append(
            {"layer": i + 1, "ph": ph_results, "betti": betti, "data": act}
        )
...
def visualize_topology_results(results, step, save_dir="results"):
    ...
    for network, name in [(results["actor"], "actor"), (results["critic"], "critic")]:
        for layer_result in network:
            ...
            plt.title(
                f"{name.capitalize()} Layer {layer_idx} Persistence Diagram at Step {step}"
            )
            plt.savefig(
                f"{save_dir}/{name}_persistence_diagram_layer{layer_idx}_{step}.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
...
def visualize_topology_over_time(topology_results, save_dir="results"):
    ...
    for dim in actor_betti_over_time:
        ...
        plt.grid(True, linestyle="--", alpha=0.7)
        plt.savefig(
            f"{save_dir}/actor_betti{dim}_over_time.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    for dim in critic_betti_over_time:
        ...
        plt.grid(True, linestyle="--", alpha=0.7)
        plt.savefig(
            f"{save_dir}/critic_betti{dim}_over_time.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    plt.grid(True, linestyle="--", alpha=0.7)
    plt.savefig(
        f"{save_dir}/total_homology_over_time.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
